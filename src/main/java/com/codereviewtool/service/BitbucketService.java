...
public class BitbucketService {
    ...
    public Optional<Set<String>> getFileTypes(String pullRequestsURL) {
        ...
        try {
            Optional<String> result = restTemplateUtil.getJSONResultByURL(pullReqActivitiesURL, headers, HttpMethod.GET, restTemplate);
            if (result.isPresent()) {
                root = objectMapper.readValue(result.get(), com.codereviewtool.common.model.bitbucket.pullrequests.diff.Root.class);
                if (root != null) {
                    List<com.codereviewtool.common.model.bitbucket.pullrequests.diff.Diff> diffs = root.getDiffs();
    ...
    public static void main(String[] args) {
        String date = "https://wfrbitbucket.int.kronos.com/rest/api/1.0/projects/wfr/repos/zeyt/pull-requests/59896/activities";
        date = date.substring(date.indexOf("pull-requests/")+"pull-requests/".length(),date.indexOf("/activities"));
    ...
    private  String getPullRequestIdFromURL(String url) {
        String endingString = url.endsWith(COMMITS) ? COMMITS : (url.endsWith(DIFF) ? DIFF : (url.endsWith(OVERVIEW) ? OVERVIEW : ""));
        return url.substring(url.indexOf("pull-requests/") + "pull-requests/".length(), url.indexOf("/" + endingString));
    }
